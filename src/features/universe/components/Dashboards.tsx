...
import type { Domain, MapData } from "../../../data/universeData";
import { KIND_LABEL } from "../../../data/universeData";
...
const CARD = "dash-card flex flex-col rounded-2xl border border-u-hair p-6 pb-[26px]";
...
function CardLabel({
  ...
  return (
    <div className="mb-5 flex items-center gap-[10px]">
      ...
      <span className="eyebrow !text-u-ink">{children}</span>
      {count !== undefined ? (
        <span className="ml-auto text-[10px] tracking-[0.1em] text-u-muted">{count}</span>
      ) : null}
    </div>
  );
}
...
function Row({
  ...
  return (
    <div className="flex flex-col gap-0.5 border-t border-u-hair py-[13px] first:border-t-0 first:pt-0">
      <div className="flex items-baseline justify-between gap-3">
        {href ? (
          <a
            ...
            className="text-[15px] font-medium text-u-ink no-underline hover:text-u-accent"
          >
            {name}
          </a>
        ) : (
          <span className="text-[15px] font-medium text-u-ink">{name}</span>
        )}
        <span className="whitespace-nowrap text-[9.5px] uppercase tracking-[0.14em] text-u-muted">
          {tag}
        </span>
      </div>
    ...
    <div className="absolute inset-x-0 bottom-0 top-16 overflow-y-auto px-[clamp(20px,5vw,72px)] pb-20 pt-[30px]">
      ...
      >
        ...
        <motion.div className="my-[14px] mb-10 max-w-[720px]" variants={item}>
          ...
          <h1 className="mb-[14px] font-u-display text-[clamp(34px,5vw,56px)] font-normal leading-[1.02]">
        ...
        <div className="grid grid-cols-12 gap-[18px]">
          ...
          <motion.div className={`${CARD} col-span-12 md:col-span-7`} variants={item}>
            ...
            {data.selectedWorks.map((w) => (
              <div
                key={w.title}
                className="flex flex-col gap-[3px] border-t border-u-hair py-[15px] first:border-t-0 first:pt-0"
              >
                {w.url ? (
                  <a
                    href={w.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-u-display text-[21px] leading-[1.1] text-u-ink no-underline hover:text-u-accent"
                  >
                    {w.title}
                  </a>
                ) : (
                  <span className="font-u-display text-[21px] leading-[1.1] text-u-ink">{w.title}</span>
                )}
                <span className="text-[11px] uppercase tracking-[0.08em] text-u-muted">{w.meta}</span>
          ...
          <motion.div className={`${CARD} col-span-12 md:col-span-5`} variants={item}>
            ...
            {data.skillGroups.map((g) => (
              <div key={g.group} className="mb-[18px] last:mb-0">
                <h4 className="m-0 mb-[10px] text-[10.5px] font-semibold uppercase tracking-[0.16em] text-u-muted">
                  {g.group}
                </h4>
            ...
            <div className="flex flex-col gap-[11px]">
              {bridges.map((b) => (
                <div key={`${b.a.id}-${b.b.id}`} className="flex items-center gap-2 text-[12.5px]">
                  ...
                  />
                  <span className="text-u-ink">{b.a.label}</span>
                  <span className="text-u-muted">↔</span>
                  <span
                    className="h-2 w-2 flex-none rounded-full"
                    style={{ background: b.b.color }}
                  />
                  <span className="text-u-ink">{b.b.label}</span>
                  <span className="ml-auto text-[10px] tracking-[0.1em] text-u-muted">{b.n}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          {/* Contact card */}
          <motion.div
            className="dash-card col-span-12 flex flex-col rounded-2xl border border-u-line p-6 pb-[26px] text-u-ink md:col-span-4"
            variants={item}
          >
            <div className="mb-5 flex items-center gap-[10px]">
              <span
            ...
            <h3 className="m-0 mb-2 font-u-display text-[28px] font-normal">{data.contact.headline}</h3>
            <p
              ...
              {data.contact.actions.map((a) => (
                ...
                >
                  <span>
                    {a.label}
                    {a.note ? (
                      <span className="ml-2 text-[9px] uppercase tracking-[0.14em] text-u-muted">
                        {a.note}
                      </span>
                    ) : null}
                  </span>
        ...
        {/* Footer */}
        <div className="mt-[46px] flex flex-wrap justify-between gap-[10px] border-t border-u-hair pt-[22px] text-[10.5px] uppercase tracking-[0.12em] text-u-muted">
